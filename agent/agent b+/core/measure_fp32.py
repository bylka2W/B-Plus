...
print("n_heads", model.n_heads, "head_dim", model.head_dim, flush=True)
it = iter(dl)
for i in range(N):
    batch = next(it)
    ...
    t4 = time.monotonic()
    print(f"step {i+1}: fwd={(t1-t0)*1000:.0f}ms bwd={(t2-t1)*1000:.0f}ms clip={(t3-t2)*1000:.0f}ms opt={(t4-t3)*1000:.0f}ms total={(t4-t0)*1000:.0f}ms", flush=True)
