...
def make_bf16():
    m = build_model_600m(vocab_size=tok.vocab_size(), max_seq_len=256000).to(DEV, dtype=torch.bfloat16)
    return m
...
def run(label, model, opt):
    ...
    print(f"[{label}] mean(2..{NSTEPS}) tot = {sum(o[0]+o[1]+o[2] for o in out[1:])/(NSTEPS-1)/1000:.0f}ms  -> {int(BS*SEQ/(sum(o[0]+o[1]+o[2] for o in out[1:])/(NSTEPS-1)))} tok/s", flush=True)

print("=== baseline (eager bf16) ===", flush=True)
m = make_bf16(); o = torch.optim.AdamW(m.parameters(), lr=3e-4, betas=(0.9,0.95), weight_decay=0.1, fused=True)
run("EAGER", m, o)

print("=== torch.compile (measure only) ===", flush=True)
m2 = make_bf16(); 
t0=time.monotonic(); m2c = torch.compile(m2, mode="default"); print(f"compile() call: {time.monotonic()-t0:.1f}s (lazy)", flush=True)
o2 = torch.optim.AdamW(m2c.parameters(), lr=3e-4, betas=(0.9,0.95), weight_decay=0.1, fused=True)
run("COMPILE", m2c, o2)
