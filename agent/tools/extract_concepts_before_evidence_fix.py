...
def extract_pipeline_concepts(claim):
    """
    Conservative extraction for explicit pipeline notation.

    Example:

        B+ source → BIR → MIR → x64 COFF object

    We only extract concepts that are explicitly present
    in the claim.

    No external knowledge is introduced here.
    """

    concepts = []

    # Look for the explicit arrow pipeline.
    parts = re.split(r"\s*[→➜⟶]\s*", claim)
    ...
    for part in parts:
        ...
        # Remove common descriptive words only when they are
        # clearly attached to the beginning/end of the phrase.
        value = re.sub(
            r"^(B\+\s+source)\s*$",
            "B+",
            value,
...
def add_relation(
    ...
    if relation_id in learned["relations"]:
        return relation_id
...
def extract_all_verified(learned):
    ...
    return total_concepts, total_relations
