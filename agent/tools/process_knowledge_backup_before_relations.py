...
def _make_evidence(evidence_db, counters, source_id, line_start, line_end, snippet):
...
def _add_relation(relations, counters, evidence_db, rel_type, source_id, fr, to, line_start, line_end, snippet):
    evid = _make_evidence(evidence_db, counters, source_id, line_start, line_end, snippet)
...
def remove_source_data(source_id, structured):
    ...
    # Dependencies belonging to source.
    ...
    # Knowledge and corresponding evidence.
    knowledge_to_remove = []
...
def process(force_rebuild=False):
    ...
    # Global symbol maps for relation detection
    global_funcs = {}
    global_structs = {}
    for kid, info in knowledge_db.items():
        if info.get("type") != "symbol_definition":
            continue
        name = info.get("symbol")
        kind = info.get("kind")
        if kind == "function":
            global_funcs[name] = info.get("file")
        elif kind == "struct":
            global_structs[name] = info.get("file")
    ...
    for index, rel in enumerate(files, 1):
        ...
        try:
            ...
            # ------------------------------------------------
                ...
                # Normal build:
                # same hash = don't touch existing knowledge.
                if (
                ...
                # Rebuild mode:
                # same hash is intentionally rebuilt.
...
def handle_query(query):
    """
    Handle a simple query against structured.json.

    Searches knowledge entries by:
      - type
      - symbol
      - name
      - kind
