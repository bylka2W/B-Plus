...
public class CppOptimizedGenerator : ICodeGenerator
{
        ...
        Dictionary<string, int> stateIds, Dictionary<string, int> eventIds)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("// Auto-generated by B+ v2.5.0GH — streaming parser");
        sb.AppendLine("// goto-driven, zero-copy, no allocations");
        sb.AppendLine();

        // Process each state and its transitions
        // For each state that has @stream, generate non-blocking variant
        ...
        sb.AppendLine("    StreamState cs = start;");
        sb.AppendLine();

        // Main event loop: direct switch with inline actions
        sb.AppendLine("    while (p < pe) {");
        sb.AppendLine("        uint8_t ch = *p;");
        sb.AppendLine("        switch (cs) {");

        foreach (var s in allStates)
        {
            sb.AppendLine($"            case ST_{s.Name}:");

            // Inline enter action (run once)
            ...
            // Non-blocking check for @stream states
            if (s.IsStream)
                sb.AppendLine($"                if (p >= pe) return ST_{s.Name}; // no data, don't block");

            // Transitions
            if (s.Transitions.Count == 0)
            {
                sb.AppendLine("                // no transitions — consume byte, stay");
                sb.AppendLine("                p++;");
                sb.AppendLine("                break;");
            }
            else
            {
                ...
                {
                    bool isByteEvent = t.EventName.Length == 1;
                    string cond;
                    if (t.IsAlways)
                    {
                        cond = "true";
                    }
                    else if (isByteEvent && t.Guard == null)
                    {
                        cond = $"ch == '{t.EventName}'";
                    }
                    else if (isByteEvent && t.Guard != null)
                    {
                        cond = $"ch == '{t.EventName}' && ({t.Guard})";
                    }
                    else if (t.Guard != null)
                    {
                        cond = $"ch == '{t.EventName[0]}' && ({t.Guard})";
                    }
                    else
                    {
                        // Multi-byte tokens: defer to stateful comparison
                        cond = $"ch == '{t.EventName[0]}'";
                    }
                    string prefix = ti == 0 ? "if" : " else if";
                    sb.AppendLine($"                {prefix} ({cond}) {{");
                ...
                sb.AppendLine("                else {");
                sb.AppendLine("                    // unknown event — stay in state, consume byte");
                sb.AppendLine("                    p++;");
                sb.AppendLine("                }");
                sb.AppendLine("                break;");
            }
        ...
        sb.AppendLine("                return ST_COUNT; // error");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();
