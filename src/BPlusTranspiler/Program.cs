...
{
    Console.WriteLine("B+ Whole-Module Optimization (WMO) Ч Swift-style");
    ...
    foreach (var bpFile in bpFiles)
    {
        ...
        {
            Console.Error.WriteLine($"  ? {bpFile}: parse error Ч {ex.Message}");
        }
    }
...
// GPU/PGO/C-ABI flags Ч declared early for watch mode closure
var gpuArch = "auto";
...
if (args.Contains("--adaptive"))
{
    ...
    Console.WriteLine("B+ Adaptive Runtime Ч generating CPU-dispatch code");
    Console.WriteLine();
    ...
    Console.WriteLine("  Х bplus_adaptive.h Ч CPU detection (CPUID) + dispatch table");
    Console.WriteLine("  Х bplus_adaptive.cpp Ч per-state dispatch + benchmark harness");
    Console.WriteLine("  Х benchmark_report.txt Ч CPU capability report");
    Console.WriteLine();
...
if (args.Contains("--verify") || args.Contains("--verify-dal-a"))
{
    ...
    Console.WriteLine("B+ Formal Verification Ч DO-178C compliance");
...
if (args.Contains("--math") || args.Contains("--math-intrinsics"))
{
    ...
    Console.WriteLine("B+ Math Intrinsics Ч AVX-512 matrix/quaternion/trig generation");
    Console.WriteLine();
    ...
    Console.WriteLine("  Х bplus_math.h Ч AVX-512 sin/cos/tan/exp/log, mat4x4, quaternion");
    Console.WriteLine("  Х bplus_math_ops.cpp Ч per-state math dispatch table");
    Console.WriteLine();
...
if (input == null)
{
    ...
    Console.Error.WriteLine("Optimization flags (реалистичные ускорени€ на state machine):");
    ...
    Console.Error.WriteLine("  --optimize                  “аблица переходов вместо virtual   +10-30%");
    Console.Error.WriteLine("  --pool                       ѕул состо€ний без new/delete       +20-40%");
    Console.Error.WriteLine("  --cache-friendly             ”пор€доченный layout данных        +10-20%");
    Console.Error.WriteLine("  --prefetch                   ѕредзагрузка кэша                  +10-20%");
    Console.Error.WriteLine("  --branchless                cmov вместо if/else в переходах    +5-15%");
    Console.Error.WriteLine("  --pack                       Ѕитфилды, упаковка структур        -40% пам€ти");
    Console.Error.WriteLine("  --predict                    ѕредсказание след. состо€ни€       +5-15%");
    Console.Error.WriteLine("  --eco                        Ёнергосбережение (узкие SIMD)");
    Console.Error.WriteLine("  --turbo                      --optimize + --pool + --pack       +40-80%");
    Console.Error.WriteLine("  --turbo-embed                ƒл€ embedded (--pack + --eco)");
    Console.Error.WriteLine("  --vectorize                  »нлайн SIMD (только с числовыми данными)");
    Console.Error.WriteLine("  --benchmark [iterations]     —генерировать бенчмарк");
    Console.Error.WriteLine("  --check / --analyze          ƒиагностика 7 категорий");
    Console.Error.WriteLine("  --lto <mode>                 Link-Time Optimization thin|full     +10-20%");
    Console.Error.WriteLine("  --thread-pool <N>            ћногопоточна€ диспетчеризаци€");
    Console.Error.WriteLine("  --lock-free                  Ѕезлоковые структуры               +5-10%");
...
// Resolve imports Ч load and merge imported .bp files, also save for separate generation
    ...
    if (optFlags.Release)
    {
        ...
        if (optFlags.Spirv)
        {
            ...
            if (File.Exists(clang))
            {
                ...
                if (sp.ExitCode == 0)
                {
                    Console.WriteLine($"Generated {bcFile} (spirv64 bitcode Ч run llvm-spirv -r for .spv)");
                }
                else
...
if (optFlags.Check || !optFlags.HasAny)
{
    ...
    // Run BPlusValidator Ч 121 checks
...
{
    Console.WriteLine("B+ v4.0.0 BETA Metal Ч NUMA + µarch + ILP + StoreFwd + AutoTune");
    Console.WriteLine();
    ...
    foreach (var mb in metalBlocks)
        foreach (var fp in mb.Config.FusionPairs)
        {
            ...
            Console.WriteLine(valid
                ...
                : $"  ? '{fp}' NOT valid on {targetMuarch} Ч remove or change @muarch");
        }
    Console.WriteLine();
    ...
    foreach (var ilp in ilpScores)
        if (ilp.MaxDependencyChain > 4)
            Console.WriteLine($"  ? {ilp.StateName}: chain={ilp.MaxDependencyChain} ILP={ilp.Score:F2} Ч {ilp.Suggestion}");
        else
...
static int RunAI(string bpFile)
{
    ...
    {
        writer.WriteLine("// B+ v4.0.0 BETA Metal Ч AI-optimized");
...
static void InstallLsp()
{
    ...
    var packageJson = new
    {
        ...
        description = "B+ state machine language Ч LSP + syntax highlighting + Snippets",
...
";
    ...
    Console.WriteLine("    Х Syntax highlighting (.bp files)");
    Console.WriteLine("    Х LSP: errors, completions, hover info, formatting");
    Console.WriteLine("    Х Code snippets: state, kernel, hot, cold, simd, comptime");
    Console.WriteLine("    Х Auto-closing pairs, bracket matching");
}
