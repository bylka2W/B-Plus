...
namespace BPlusTranspiler.Verification;
...
public class FormalVerifier
{
    ...
    // 1. S1 — Все состояния достижимы из начального
    ...
    // 2. S2 — Детерминизм переходов (один обработчик на событие)
    ...
    // 3. S3 — Отсутствие тупиков (deadlock)
    ...
    // 5. S5 — Memory safety (no dangling writes)
    ...
    // 6. S6 — Независимость параллельных блоков
    ...
    // 7. S7 — Чистота guards (нет side-effects)
    ...
    // 8. S8 — Терминация (нет бесконечных циклов always → self)
    ...
    // 9. S9 — Отсутствие двойных entry points
    ...
    // 10. S10 — Покрытие: все состояния/переходы
    void VerifyCoverage()
    {
        foreach (var s in _allStates)
        {
    ...
    public string GenerateReport(SafetyLevel level = SafetyLevel.DAL_C)
    {
        ...
        sb.AppendLine("╔══════════════════════════════════════════════════════╗");
        sb.AppendLine("║  B+ Formal Verification Report — DO-178C           ║");
        sb.AppendLine("╚══════════════════════════════════════════════════════╝");
        sb.AppendLine();
