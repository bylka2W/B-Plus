    ...
    public static void Run(string[] args)
    {
        ...
        while (true)
        {
            ...
            if (trimmed == ".update")
            {
                ...
                var ok = RunCommand("dotnet", "publish src/BPlus.Cli -c Release -r win-x64 --self-contained -p:PublishSingleFile=true", silent: true);
                if (!ok)
                {
                ...
                var publishDir = Path.GetFullPath("src/BPlus.Cli/bin/Release/net8.0/win-x64/publish", rootDir);
