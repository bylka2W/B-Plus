...
public static class BPlusBenchRunner
{
    public static int Run(string bpFile, int iterations)
    {
        ...
        var prog = new BPlus.Core.Parser.BPlusParser().Parse(src);
...
"print(f'OPS:{ops:.2f}')\n";
        ...
        Console.WriteLine($"  Avg: {(avg*1_000_000).ToString("F0", ic)}�s per run");
        Console.WriteLine($"  Min: {(min*1_000_000).ToString("F0", ic)}�s");
        Console.WriteLine($"  Max: {(max*1_000_000).ToString("F0", ic)}�s");
    ...
    {
        // Normalize boolean literals for Python: = false > = False, = true > = True
        code = System.Text.RegularExpressions.Regex.Replace(code, @"\bfalse\b", "False");
        code = System.Text.RegularExpressions.Regex.Replace(code, @"\btrue\b", "True");
        // Normalize null > None
        code = System.Text.RegularExpressions.Regex.Replace(code, @"\bnull\b", "None");
        return code;
    }
}
