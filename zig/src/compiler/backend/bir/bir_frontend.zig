...
const gpu_ast = @import("../../parser/gpu_ast.zig");
const gpu_body_parser = @import("../../parser/gpu_body_parser.zig");
...
fn convertIrFunction(allocator: Allocator, bir_mod: *bir.Module, func_id: bir.FunctionId, ir_func: *const gpu_ir.IrFunction, type_map: *const std.AutoHashMap(gpu_ir.TypeRef, TypeId)) !void {
    ...
    for (0..total_insts) |_| _ = fn_ptr.createValue();
    ...
    var next_new_val: ValueId = 1;
        ...
        for (0..old_block_count) |_b_idx| {
            ...
            for (old_block.instrs.items) |*old_inst| {
                ...
                switch (bir_inst.data) {
                    .texture_store_info => |tsi| {
                        if (tsi.tex != NO_VALUE) try fn_ptr.getValueInfo(tsi.tex).uses.append(bir_inst.result);
                        if (tsi.coord_x != NO_VALUE) try fn_ptr.getValueInfo(tsi.coord_x).uses.append(bir_inst.result);
                        if (tsi.coord_y != NO_VALUE) try fn_ptr.getValueInfo(tsi.coord_y).uses.append(bir_inst.result);
                        if (tsi.val != NO_VALUE) try fn_ptr.getValueInfo(tsi.val).uses.append(bir_inst.result);
                    },
                    .named_call => |nc| {
                        for (nc.args) |arg| {
                            if (arg != NO_VALUE) try fn_ptr.getValueInfo(arg).uses.append(bir_inst.result);
                        }
                    },
                    else => {},
                }
