...
pub fn parseStmt(allocator: Allocator, line: []const u8) ?hir.Stmt {
    if (std.mem.startsWith(u8, line, "return")) {
        ...
        const expr = parseExpr(allocator, rest);
    ...
    if (std.mem.indexOfScalar(u8, line, '=')) |eq_idx| {
        ...
        if (lhs.len > 0 and rhs.len > 0 and isIdent(lhs)) {
            const value = parseExpr(allocator, rhs);
            return .{ .assign = .{ .name = lhs, .value = value orelse return null } };
        }
    }
...
pub fn parseExpr(allocator: Allocator, expr: []const u8) ?hir.Expr {
...
pub const BraceBlock = struct { body_start: usize, body_end: usize };

pub fn findBraceBlock(text: []const u8) ?BraceBlock {
    var i: usize = 0;
...
pub const BinParts = struct { left: []const u8, right: []const u8 };

pub fn findBinOp(expr: []const u8, op: []const u8) ?BinParts {
    var depth: i32 = 0;
...
pub fn findParenEnd(line: []const u8, open: usize) ?usize {
