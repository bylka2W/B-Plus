...
const frontend = @import("../../frontend/ast.zig");
...
const Ctx = struct {
    ...
    program: *const frontend.ProgramNode,
...
pub fn generate(allocator: std.mem.Allocator, program: frontend.ProgramNode, source: []const u8) !HlslOutput {
...
fn emitConstantBufferDecl(w: anytype, program: *const frontend.ProgramNode, bindings: *const std.StringHashMap([]const u8)) !void {
    var first = true;
...
fn emitEntryShader(w: anytype, entry: *const frontend.EntryDecl, ctx: *Ctx) @TypeOf(w).Error!void {
    var nt_x: u32 = 8;
    var nt_y: u32 = 8;
    const nt_z: u32 = 1;
...
fn isStateVar(name: []const u8, program: *const frontend.ProgramNode) bool {
    for (program.states.items) |*state| {
        for (state.variables.items) |*v| {
...
fn inferType(name: []const u8, program: *const frontend.ProgramNode) []const u8 {
    for (program.states.items) |*state| {
        for (state.variables.items) |*v| {
