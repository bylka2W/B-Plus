...
fn runUnroll(module: *bir.Module, allocator: Allocator) anyerror!void {
    _ = allocator;
...
fn unrollLoop(module: *bir.Module, func_id: bir.FunctionId, func: *bir.Function, cfg: *const bir_cfg.CFG, _: *const bir_dominators.DominatorTree, loop: *const bir_loops.Loop) !void {
        ...
        while (iter < count) : (iter += 1) {
            ...
            for (body_blocks.items) |bid| {
                ...
                for (src_block.instrs.items) |*inst| {
                    ...
                    const new_val = func.createValue();
