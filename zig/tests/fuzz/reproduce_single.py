#!/usr/bin/env python3
"""Reproduce crash at seed=123, ID=42 with full fuzzer logic."""
...
def rand_expr(depth=0):
    if depth > 5 or rng.random() < 0.3:
        if rng.random() < 0.5: return rand_num()
        return rand_ident()
...
for test_id in range(target + 1):
    ...
    if test_id == target:
        ...
        try:
            ...
            if stdout: print(f"stdout: {stdout[:500]}")
